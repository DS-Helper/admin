...
class CreateOrganizationReservationReqDtoTest {

    @Test
    @DisplayName("기관 예약 요청은 엔티티로 변환된다")
    void toOrganizationReservation_mapsFields() {
        CreateOrganizationReservationReqDto dto = new CreateOrganizationReservationReqDto();
        ...
        dto.setRecipientGenderType("남");
        dto.setRecipientNumber(2);
        dto.setNote(null);

        OrganizationReservation reservation = dto.toOrganizationReservation(dto, User.builder().id("user-1").build());

        assertThat(reservation.getRecipientGender()).isEqualTo(RecipientGenderType.MALE);
