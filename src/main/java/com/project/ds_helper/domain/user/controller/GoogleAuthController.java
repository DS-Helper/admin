...
public class GoogleAuthController {
    ...
    @Operation(summary = "구글 로그인 URL 조회", description = "구글 OAuth 로그인 페이지로 이동하기 위한 URL을 조회합니다.")
    @GetMapping("/login-url")
    public ResponseEntity<String> getGoogleLoginUrl() {
        return ResponseEntity.ok(googleOAuthService.getGoogleLoginUrl());
    }

    @Operation(summary = "구글 로그인", description = "구글 authorization code를 받아 사용자 정보 조회 후 로그인 또는 회원가입을 처리합니다.")
    @PostMapping("/login")
    public ResponseEntity<ResponseVo<JwtResponse>> googleOauthLogin(
            @RequestBody @Valid GoogleCodeLoginRequestDto dto
    ) throws IOException {
        JwtResponse jwtResponse = googleOAuthService.googleOauthLogin(dto.code());
