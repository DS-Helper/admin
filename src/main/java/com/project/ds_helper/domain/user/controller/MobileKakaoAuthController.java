...
public class MobileKakaoAuthController {

    private final MobileKakaoOauthService mobileKakaoOauthService;

    @Operation(summary = "모바일 카카오 로그인", description = "모바일 카카오 access token을 받아 로그인 또는 회원가입을 처리합니다.")
    @PostMapping("/oauth/kakao/login")
    public ResponseEntity<ResponseVo<JwtResponse>> mobileKakaoLogin(@RequestBody @Valid MobileKakaoLoginRequestDto dto,
                                                       HttpServletResponse httpServletResponse) throws IOException {
        JwtResponse jwtResponse = mobileKakaoOauthService.mobileKakaoLogin(dto, httpServletResponse);
